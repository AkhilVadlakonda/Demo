...
public class ProductDao {
	...
	public ArticleUsersResponse ActiveUsers(Integer page, HttpServletRequest request) throws JsonProcessingException {
		...
		ArticleUsersResponse response =  new ArticleUsersResponse();
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
		for(int i=1;i<=page;i++)
		{
		String url = "https://jsonmock.hackerrank.com/api/article_users?page=" + i;
		try {
			HttpEntity<Object> requestEntity = new HttpEntity<>(headers);
			ResponseEntity<ArticleUsersResponse> responseEntity = retryService.hackerrankCall(url,requestEntity);
			response = responseEntity.getBody();
			if(CollectionUtils.isEmpty(response1.getData()))
			{
				response1.setData(response.getData());
			}
			else
			{
